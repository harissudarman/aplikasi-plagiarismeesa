...
def combined_similarity(text1, text2):
    ...
    # Jika kemiripan Levenshtein cukup tinggi, lanjutkan dengan TF-IDF
    if levenshtein_score > 0.7:  # Ambang batas kemiripan Levenshtein
        vectorizer = TfidfVectorizer().fit_transform([text1, text2])
        vectors = vectorizer.toarray()
        cosine_score = cosine_similarity(vectors)[0, 1]
        final_score = (levenshtein_score + cosine_score) / 2  # Gabungkan skor
    else:
        final_score = levenshtein_score  # Hanya gunakan skor Levenshtein

    return final_score * 100  # Kembalikan dalam persen
